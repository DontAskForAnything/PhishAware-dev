...
const data = [
  ...
  {name: "monthly_participation_rate", title: "72%", change: "2%", direction: "down"},
        ...
        {data.map((item) => (
          ...
          <div className="card-body gap-2">
            <div className="flex items-start justify-between flex-col gap-2 text-sm">
                ...
                <p className="text-base-content/80 font-medium uppercase">{item.name.split("_").join(' ')}</p>
                <div className="mt-3 flex items-center gap-2">
                  <p className="text-2xl font-semibold">{item.title}</p>
                  {item.change && <div className={`badge badge-soft  ${item.direction === 'down' ? "badge-error" :  "badge-success"} badge-sm gap-0.5 px-1 font-medium text-base`}>
