const dane = {
  wyniki: [
    { name: "Bezpieczeństwo urządzeń", value: "72%" },
    { name: "Hasła", value: "35%" },
  ...
  summary: [
    { name: "Średni wynik", value: "60%", color: "bg-[#F6B137]" },
    {
      name: "Miesięczny wskaźnik uczestnictwa",
      value: "96%",
      color: "bg-[#378075]",
    },
  ...
  najbardziej_aktywni: [
    { name: "Michał Jasiński", value: "240 000 hektarów" },
  ...
  najlepszy_sredni_wynik: [
    { name: "Liwia Cieślak", value: "92%" },
      ...
      <div className="w-full   bg-[#D9D9D9]  p-6  gap-4 drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)]   flex flex-col rounded-4xl bg-[url(/bg-sign-3.png)]  bg-cover bg-no-repeat  bg-left">
        {dane.summary.map((item, index) => (
          <div key={index} className="flex justify-between items-center ">
            ...
            <div
              className={`${item.color ? `${item.color} text-white` : "text-black font-semibold"}  text-sm rounded-lg px-4 py-2 w-32 text-center`}
            >
              {item.value}
            </div>
          </div>
          ...
          {dane.wyniki.map((item, index) => (
            <div key={index} className="w-1/5 flex flex-col items-center">
              <div className="h-80  w-[70px] flex flex-col-reverse bg-[url(/lines.png)]   rounded-t-2xl">
                {/* Bar */}
                <div
                  className={` text-white  text-center flex items-end justify-center
                                
                                      py-2 rounded-t-2xl ${
                                        parseFloat(item.value) > 70
                                          ? "bg-[#378075]"
