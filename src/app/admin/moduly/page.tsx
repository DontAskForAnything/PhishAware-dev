  ...
  return (
    <div className=" px-6 pb-6 gap-8 flex flex-col">
      ...
      <div className="  grid grid-cols-2 rows-2 gap-8">
        {dane.map((item, index) => (
          <div key={index} className={`w-full  gap-4  bg-[#D9D9D9]  p-8 drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)]   flex flex-col rounded-4xl ${!item.active && "opacity-70"} `}>
            <div className="flex flex-1 justify-between items-center  " >
              <div className="font-bold text-xl ">{item.name}</div>
           
           {item.active ?    <div className="rounded-2xl w-28 h-10 py-[20px] px-1 bg-transparent border border-[#024840] flex justify-center items-center">
